...
def get_labels(data, fill=["number"], max_elements=3):
    """
    ...
    input
      ...
      max_elements: maximum number of elements to display per region
    ...
    # Mostrar elementos aleatorios
    if "elements" in fill:
        for k in set_collections:
            elements = list(set_collections[k])
            if len(elements) > max_elements:
                elements = random.sample(elements, max_elements)
